        ...
        if (serverEnabled() && playerEnabled(event.getPlayer())) {
            if (event.getAction().toString().startsWith("RIGHT_") && event.getItem() != null && event.getHand() != null) {
                if (event.getItem().getType() == Material.BOW) {
                    ...
                    ItemStack stack = playerInv.getItem(InvUtil.getEquipmentSlotId(event.getPlayer(), targetHand)).clone();
                    playerInv.setItem(InvUtil.getEquipmentSlotId(event.getPlayer(), targetHand), ItemUtil.getItemStack(Material.ARROW, "&7&o无限附魔辅助箭", "更好的无限附魔"));
                    ...
                    Bukkit.getScheduler().runTaskLater(InvActions.getInstance(), () -> {
                        event.getPlayer().getInventory().setItem(InvUtil.getEquipmentSlotId(event.getPlayer(), targetHand), stack);
